...
class WideResNet(nn.Module):
    def __init__(self, num_classes, depth=28, widen_factor=2, dropout=0.0, dense_dropout=0.0):
        ...
        print('dropouts are ', dropout, dense_dropout)
...
class SimpleNet(nn.Module):
    ...
    def __init__(self):
        ...
        self.fc1 = nn.Linear(2, 120).double() 
        self.fc2 = nn.Linear(120, 84).double()
        self.fc3 = nn.Linear(84, 2).double()
...
def build_simplenet(args):
    model = SimpleNet()
    return model

def get_model(args):
    if args.dataset == "TWOMOON":
        model = build_simplenet(args)
    elif args.dataset == "CIFAR10":
        model = build_wideresnet(args)
    return model

def initialize_model(args):
    if args.model == "wideResNet-28-2":
        depth, widen_factor = 28, 2
        model = WideResNet(num_classes=args.classes,
                       depth=depth,
                       widen_factor=widen_factor,
                       dropout=args.dropout,
                       dense_dropout=args.dense_dropout)
